...
function Field({ label, children, className = "", required = false }: { label: string; children: ReactNode; className?: string; required?: boolean }) {
  return <label className={`portfolio-field ${className}`}><span>{required && <b className="required-mark" aria-hidden="true">*</b>}{label}</span>{children}</label>;
}
...
function PolicyForm({ policy, onSaved }: { policy: Dict; onSaved: () => void }) {
  ...
  return <details className="portfolio-policy-editor"><summary>Create Policy Version (expected_version={text(policy.version)})</summary><div className="portfolio-policy-form">{POLICY_FIELDS.map(({ key, label, step }) => <Field label={label} key={key} required><input required type="number" step={step ?? "0.01"} value={values[key]} onChange={(event) => setValues((current) => ({ ...current, [key]: event.target.value }))} /></Field>)}</div>{error && <div className="inline-error">{error}</div>}<div className="portfolio-form-actions"><ActionButton onClick={() => { void save(); }} busy={busy}>Confirm Append</ActionButton><small className="muted">User confirmation, idempotency key, and expected_version are recorded in the audit trail.</small></div></details>;
}
