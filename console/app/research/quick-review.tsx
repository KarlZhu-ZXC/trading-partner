...
type Snapshot = {
  ...
  latest_thinking: Array<{ note_id: string; note_revision_id: string; version: number; title: string; source_timestamp: string | null; observed_at: string; status: string; user_excerpt: string; user_summary: string; other_viewpoints: Array<{ speaker: string; summary: string }>; warnings: string[] }>;
...
export function QuickReview({ subjectId, enabled, onAdjustThesis, onAdjustPlan }: { subjectId: string; enabled: boolean; onAdjustThesis: (seed?: { statement: string; rationale: string; sourceRevisionId: string }) => void; onAdjustPlan: () => void }) {
  ...
  async function load(afterNoteRefresh = false) {
    ...
    try {
      ...
      setSnapshot(response.data); setNow(Date.now()); setNotesChanged(false); setNotesUpdated(afterNoteRefresh);
  ...
  useEffect(() => {
    const changed = () => {
      if (enabled && !sending && !result) { void load(true); }
  ...
  }, [enabled, sending, result, subjectId]);
  ...
  async function submit() {
    if (!snapshot || readFailed || loading || sending || expired || invalid || result) return;
    const body = { review_token: snapshot.review_token, baseline_decision_id: snapshot.baseline?.decision_id ?? null, action, rationale: rationale.trim(), review_due_at: action === "defer" ? dueIso : null, confirmed: true };
    const signature = JSON.stringify(body);
    if (intent.current?.signature !== signature) intent.current = { signature, key: crypto.randomUUID() };
    setSending(true); setError(null);
    try {
      const response = await postApi<{ data: { decision_id: string; action: string; status: string; review_due_at: string | null } }>(`/api/research/${encodeURIComponent(subjectId)}/quick-review`, { ...body, idempotency_key: intent.current.key });
      if (!response.data?.decision_id || response.data.status !== "RECORDED") throw new Error("The recorded outcome could not be verified. Retry the same intent to recover it.");
      setResult(response.data);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Submission outcome unavailable. Retry the same intent to recover it."); }
  ...
  return <Card kicker="REVIEWED JUDGMENT" title={snapshot?.title ?? "Quick Review"}>
    ...
    {!result && <Button disabled={loading || sending} onClick={() => { void load(); }}>Refresh Review Context</Button>}
    {snapshot && <>
      ...
      {(snapshot.latest_thinking ?? []).map((thinking) => <div key={thinking.note_revision_id}><h3>{thinking.title}</h3><p>{formatDate(thinking.source_timestamp ?? thinking.observed_at)} · {thinking.status === "EXTRACTED" ? "Thinking extracted" : thinking.status === "PENDING" ? "Interpretation pending" : thinking.status === "SUMMARY_ONLY" ? "Summary available" : "Interpretation unavailable"}</p><p>{thinking.user_summary || thinking.user_excerpt || "No attributed thinking is available for this note yet."}</p>{thinking.user_summary && thinking.user_excerpt && <Disclosure title="Original Excerpt"><blockquote>{thinking.user_excerpt}</blockquote></Disclosure>}{thinking.other_viewpoints.length > 0 && <Disclosure title="Quoted Others">{thinking.other_viewpoints.map((other, index) => <p key={index}>{other.speaker}: {other.summary}</p>)}</Disclosure>}{thinking.warnings.length > 0 && <p>Some source details need attention; inspect Source Details below.</p>}{(thinking.user_summary || thinking.user_excerpt) && <Button disabled={sending} onClick={() => onAdjustThesis({ statement: thinking.user_summary || thinking.user_excerpt, rationale: `Moomoo USER thinking for review. Instrument ${snapshot.instrument_id}; note ${thinking.note_id}; revision ${thinking.note_revision_id}; v${thinking.version}; source ${thinking.source_timestamp ?? "unknown"}; observed ${thinking.observed_at}; status ${thinking.status}.`, sourceRevisionId: thinking.note_revision_id })}>Review Thinking in Thesis</Button>}</div>)}
      <Button disabled={refreshingNotes || sending} onClick={async () => { setRefreshingNotes(true); try { await startObservationRefresh(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Note refresh failed"); } finally { setRefreshingNotes(false); } }}>Refresh Notes</Button>
    ...
    {result ? <div aria-live="polite"><p>Recorded {result.action === "maintain" ? "NO_ACTION" : "Follow-up"}{result.review_due_at ? ` · Due ${formatDate(result.review_due_at)}` : ""}</p><Disclosure title="Decision Receipt"><p>{result.decision_id}</p></Disclosure><Button onClick={() => { setResult(null); intent.current = null; setAction("maintain"); setRationale(DEFAULT_REASON); setDue(""); void load(); }}>Review Next</Button></div> : <>
      ...
      <label><FieldLabel required>Review Action</FieldLabel><Select required disabled={sending} value={action} onChange={(event) => { const next = event.target.value as "maintain" | "defer"; setAction(next); if (next === "defer" && rationale === DEFAULT_REASON) setRationale(""); }}><option value="maintain">Maintain Prior Judgment</option><option value="defer">Defer for Missing Evidence</option></Select></label>
      <label><FieldLabel required>{action === "defer" ? "Evidence Gap / Follow-up Reason" : "Review Rationale"}</FieldLabel><Textarea required disabled={sending} value={rationale} onChange={(event) => setRationale(event.target.value)} /></label>
      {action === "defer" && <label><FieldLabel required>Follow-up Date (Local Time)</FieldLabel><Input required type="datetime-local" disabled={sending} value={due} onChange={(event) => setDue(event.target.value)} /></label>}
      ...
      <Button disabled={readFailed || loading || sending || expired || invalid || (!snapshot)} onClick={() => { void submit(); }}>{sending ? "Saving…" : error && intent.current ? "Retry Same Review" : action === "maintain" ? "Confirm No Action" : "Save Follow-up"}</Button>
    </>}
  </Card>;
}
