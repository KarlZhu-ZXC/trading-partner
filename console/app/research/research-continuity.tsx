...
function ChallengeWorkspace({ subject }: { subject: Dict }) {
  ...
  return <Card className="research-challenge-card" kicker="STRICT REVIEW · NON-EXECUTING" title="Challenge Review"><div className="challenge-controls"><label><span>Review ID</span><input value={reviewId} onChange={(event) => setReviewId(event.target.value)} placeholder="challenge_<uuid7>" /></label><ActionButton busy={busy} onClick={() => { if (reviewId.trim()) void invoke("challenge_review_get", { review_id: reviewId.trim() }); }}>Load</ActionButton><label><span>Trigger</span><select value={trigger} onChange={(event) => setTrigger(event.target.value)}>{["discussion", "thesis_activation", "confidence_increase", "invalidation_relaxation", "position_intent", "contrary_evidence", "position_thesis_conflict", "stale_review", "confidence_without_evidence"].map((item) => <option key={item}>{item}</option>)}</select></label><label className="challenge-action"><span>Proposed action</span><input value={proposedAction} onChange={(event) => setProposedAction(event.target.value)} /></label><ActionButton busy={busy} onClick={() => { if (!proposedAction.trim()) { window.alert("Enter a proposed action."); return; } void invoke("challenge_review_manage", { operation: "start", case_id: subject.subject_id, trigger, proposed_action: proposedAction.trim(), related_evidence_ids: [], idempotency_key: trigger === "discussion" ? null : key("challenge") }, "challenge_review_manage"); }}>Start Review</ActionButton></div>{error && <div className="inline-error">{error}</div>}{review && <div className="challenge-review"><header><div><strong>{value(review.review_id)}</strong><small>{value(review.trigger)} · {formatDate(review.created_at)}</small></div><Badge value={value(review.status).toUpperCase()} /></header><p>{value(review.proposed_action)}</p><div className="challenge-questions">{listOf<Dict>(review, "questions").map((question) => <div key={value(question.question_id)}><span>{value(question.dimension)}</span><p>{value(question.prompt)}</p></div>)}</div>{value(review.status, "").toLowerCase() === "open" && <div className="challenge-resolution"><select value={resolution} onChange={(event) => setResolution(event.target.value)}>{["accept", "revise", "reject", "defer"].map((item) => <option key={item}>{item}</option>)}</select><input value={rationale} onChange={(event) => setRationale(event.target.value)} placeholder="Resolution rationale (required)" /><ActionButton busy={busy} onClick={() => { if (!rationale.trim()) { window.alert("Enter a resolution rationale."); return; } if (window.confirm("Record this non-executing Review outcome?")) void invoke("challenge_review_manage", { operation: "resolve", review_id: review.review_id, resolution, rationale: rationale.trim(), confirmed_by: "user", idempotency_key: key("challenge-resolve") }, "challenge_review_manage"); }}>Record outcome</ActionButton></div>}</div>}</Card>;
}
