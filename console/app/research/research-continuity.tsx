...
function TradePlanWorkspace({ subject, state, onWrite, onRefresh, busy }: { subject: Dict; state: Dict; onWrite: Write; onRefresh: () => void; busy: boolean }) {
  ...
  return <Card className="research-trade-plan-card" kicker="POSITION INTENT · VERSIONED · NON-EXECUTING" title="Trade Plan" action={<div className="research-detail-actions">{current && <Badge value={`V${value(current.version)} · ${value(current.status)}`} />}<ActionButton onClick={openEditor}>{current ? "Propose new version" : "Create Trade Plan"}</ActionButton></div>}>
    {current ? <>
    ...
    {editing && <div className="continuity-editor"><div className="research-form-grid"><label className="research-field"><span>Bound Thesis</span><select value={thesisId} onChange={(event) => setThesisId(event.target.value)}><option value="">Select one</option>{liveTheses.map((item) => <option value={value(item.thesis_id)} key={value(item.thesis_id)}>{value(item.title)} · {shortId(item.thesis_id)}</option>)}</select></label><label className="research-field"><span>Plan status</span><select value={status} onChange={(event) => setStatus(event.target.value)}>{["DRAFT", "ACTIVE", "PAUSED", "ARCHIVED"].map((item) => <option key={item}>{item}</option>)}</select></label><label className="research-field"><span>Execution / position instrument</span><input value={instrument} onChange={(event) => setInstrument(event.target.value)} /></label><label className="research-field"><span>Currency</span><input value={currency} onChange={(event) => setCurrency(event.target.value)} /></label><label className="research-field"><span>Reference price</span><input type="number" step="any" value={referencePrice} onChange={(event) => setReferencePrice(event.target.value)} /></label><label className="research-field"><span>Reference price time</span><input type="datetime-local" value={referenceAt} onChange={(event) => setReferenceAt(event.target.value)} /></label><label className="research-field"><span>Valid from</span><input type="datetime-local" value={validFrom} onChange={(event) => setValidFrom(event.target.value)} /></label><label className="research-field"><span>Valid until (optional)</span><input type="datetime-local" value={validUntil} onChange={(event) => setValidUntil(event.target.value)} /></label><label className="research-field"><span>Target position %</span><input type="number" min="0" max="100" step="any" value={target} onChange={(event) => setTarget(event.target.value)} /></label><label className="research-field"><span>Max position %</span><input type="number" min="0" max="100" step="any" value={maximum} onChange={(event) => setMaximum(event.target.value)} /></label><label className="research-field"><span>Risk budget %</span><input type="number" min="0" max="100" step="any" value={riskBudget} onChange={(event) => setRiskBudget(event.target.value)} /></label><label className="research-field"><span>Stop price (optional)</span><input type="number" step="any" value={stop} onChange={(event) => setStop(event.target.value)} /></label><label className="research-field research-field-wide"><span>Plan notes</span><textarea rows={4} value={notes} onChange={(event) => setNotes(event.target.value)} /></label></div>
      <div className="continuity-heading"><strong>Plan conditions</strong><button type="button" className="close-button" onClick={() => setConditions((items) => [...items, { ...EMPTY_CONDITION }])}>Add condition</button></div>{conditions.map((item, index) => <div className="trade-plan-condition-editor" key={`${index}-${item.conditionCode}`}><input aria-label="Condition code" placeholder="CONDITION_CODE" value={item.conditionCode} onChange={(event) => updateCondition(index, "conditionCode", event.target.value)} /><select aria-label="Phase" value={item.phase} onChange={(event) => updateCondition(index, "phase", event.target.value)}>{["ENTRY", "SCALE", "EXIT", "INVALIDATION", "REVIEW"].map((option) => <option key={option}>{option}</option>)}</select><select aria-label="Mode" value={item.mode} onChange={(event) => updateCondition(index, "mode", event.target.value)}>{["MANUAL", "MONITORABLE"].map((option) => <option key={option}>{option}</option>)}</select><select aria-label="Severity" value={item.severity} onChange={(event) => updateCondition(index, "severity", event.target.value)}>{["INFO", "MEDIUM", "HIGH"].map((option) => <option key={option}>{option}</option>)}</select><textarea aria-label="Condition meaning" placeholder="Specific, understandable condition meaning" value={item.description} onChange={(event) => updateCondition(index, "description", event.target.value)} />{item.mode === "MONITORABLE" && <><select aria-label="Fact type" value={item.factType} onChange={(event) => updateCondition(index, "factType", event.target.value)}>{["PRICE", "VOLUME", "TECHNICAL", "FUNDAMENTAL", "COMPANY_EVENT", "MACRO", "SENTIMENT", "THESIS_STATE", "PORTFOLIO_RISK"].map((option) => <option key={option}>{option}</option>)}</select><input aria-label="Metric key" value={item.metricKey} onChange={(event) => updateCondition(index, "metricKey", event.target.value)} /><select aria-label="Comparator" value={item.comparator} onChange={(event) => updateCondition(index, "comparator", event.target.value)}>{["GT", "GTE", "LT", "LTE", "EQ", "OCCURRED"].map((option) => <option key={option}>{option}</option>)}</select><input aria-label="Threshold" type="number" step="any" disabled={item.comparator === "OCCURRED"} value={item.threshold} onChange={(event) => updateCondition(index, "threshold", event.target.value)} /><input aria-label="Unit" value={item.unit} onChange={(event) => updateCondition(index, "unit", event.target.value)} /></>}<button type="button" className="close-button warning-text" onClick={() => setConditions((items) => items.filter((_, itemIndex) => itemIndex !== index))}>Remove</button></div>)}
...
function ResearchMemoryWorkspace({ subject, onWrite, busy }: { subject: Dict; onWrite: Write; busy: boolean }) {
  ...
  return <Card className="research-memory-card" kicker="DURABLE MEMORY" title="Timeline, Journal & Decisions" action={<div className="research-detail-actions"><button className="close-button" type="button" onClick={() => setMode("journal")}>Add journal entry</button><button className="close-button" type="button" onClick={() => setMode("decision")}>Record decision</button></div>}>
    {mode && <div className="continuity-editor"><div className="research-form-grid"><label className="research-field"><span>{mode === "journal" ? "Entry type" : "Decision type"}</span>{mode === "journal" ? <select value={entryType} onChange={(event) => setEntryType(event.target.value)}>{["note", "observation", "reflection", "postmortem", "question"].map((item) => <option key={item}>{item}</option>)}</select> : <select value={decisionType} onChange={(event) => setDecisionType(event.target.value)}>{["watch", "no_action", "initiate_intent", "add_intent", "hold", "reduce_intent", "exit_intent", "avoid", "research_more"].map((item) => <option key={item}>{item}</option>)}</select>}</label><label className="research-field"><span>Title</span><input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label className="research-field research-field-wide"><span>{mode === "journal" ? "Body (Markdown)" : "Decision rationale"}</span><textarea rows={5} value={body} onChange={(event) => setBody(event.target.value)} /></label></div><div className="research-form-actions"><ActionButton busy={busy} onClick={() => { void submit(); }}>Save</ActionButton><button className="close-button" type="button" onClick={() => setMode(null)}>Cancel</button></div></div>}
...
function ChallengeWorkspace({ subject }: { subject: Dict }) {
  ...
  return <Card className="research-challenge-card" kicker="STRICT REVIEW · NON-EXECUTING" title="Challenge Review"><div className="challenge-controls"><label><span>Review ID</span><input value={reviewId} onChange={(event) => setReviewId(event.target.value)} placeholder="challenge_<uuid7>" /></label><ActionButton busy={busy} onClick={() => { if (reviewId.trim()) void invoke("research_judgment_get", { operation: "challenge_review", review_id: reviewId.trim() }); }}>Load</ActionButton><label><span>Trigger</span><select value={trigger} onChange={(event) => setTrigger(event.target.value)}>{["discussion", "thesis_activation", "confidence_increase", "invalidation_relaxation", "position_intent", "contrary_evidence", "position_thesis_conflict", "stale_review", "confidence_without_evidence"].map((item) => <option key={item}>{item}</option>)}</select></label><label className="challenge-action"><span>Proposed action</span><input value={proposedAction} onChange={(event) => setProposedAction(event.target.value)} /></label><ActionButton busy={busy} onClick={() => { if (!proposedAction.trim()) { window.alert("Enter a proposed action."); return; } void invoke("research_judgment_propose", { operation: "challenge_review", case_id: subject.subject_id, trigger, proposed_action: proposedAction.trim(), related_evidence_ids: [], idempotency_key: trigger === "discussion" ? null : key("challenge") }, "research_judgment_propose"); }}>Start Review</ActionButton></div>{error && <div className="inline-error">{error}</div>}{review && <div className="challenge-review"><header><div><strong>{value(review.review_id)}</strong><small>{value(review.trigger)} · {formatDate(review.created_at)}</small></div><Badge value={value(review.status).toUpperCase()} /></header><p>{value(review.proposed_action)}</p><div className="challenge-questions">{listOf<Dict>(review, "questions").map((question) => <div key={value(question.question_id)}><span>{value(question.dimension)}</span><p>{value(question.prompt)}</p></div>)}</div>{value(review.status, "").toLowerCase() === "open" && <div className="challenge-resolution"><select value={resolution} onChange={(event) => setResolution(event.target.value)}>{["accept", "revise", "reject", "defer"].map((item) => <option key={item}>{item}</option>)}</select><input value={rationale} onChange={(event) => setRationale(event.target.value)} placeholder="Resolution rationale (required)" /><ActionButton busy={busy} onClick={() => { if (!rationale.trim()) { window.alert("Enter a resolution rationale."); return; } if (window.confirm("Record this non-executing Review outcome?")) void invoke("research_judgment_confirm", { operation: "challenge_review", review_id: review.review_id, resolution, rationale: rationale.trim(), confirmed_by: "user", idempotency_key: key("challenge-resolve") }, "research_judgment_confirm"); }}>Record outcome</ActionButton></div>}</div>}</Card>;
}
