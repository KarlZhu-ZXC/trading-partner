...
import { ConfirmationDialog, Disclosure, ErrorNote, ActionButton, Badge, Card, DataBoundary, Empty, HorizontalTabs, PageActionMenu, displayJson, formatDate, formatDecimal, monitorAnchorId, shortId,
  Button,
  Input,
  LinkButton,
  SelectableRow,
  ...
  return (
    <section className={`monitor-judgment-card ${urgency.toLowerCase()}`}>
      ...
      {(evidence.length > 0 || sources.length > 0 || warnings.length > 0 || errors.length > 0) && (
        <Disclosure
          className="monitor-judgment-evidence"
          title={`Evidence & Diagnostics · ${evidence.length + sources.length + warnings.length + errors.length}`}
          variant="compact"
        >
          ...
          {sources.length > 0 && <div className="monitor-judgment-sources">
            ...
            {sources.map((url) => <LinkButton href={url} key={url} rel="noreferrer" target="_blank">{url}</LinkButton>)}
          </div>}
...
export default function MonitorsPage() {
  ...
  const selectedAttentionRules = selectedRules.flatMap((rule) => {
    const state = selectedStatesByCode.get(String(rule.rule_code));
    return state && String(state.state ?? "").toUpperCase() !== "QUIET"
      ? [{ rule, state }]
      : [];
  });
  ...
  return (
    <ConsoleShell active="monitors" pageActions={<PageActionMenu ariaLabel="Monitors Page Actions" items={[
      ...
      <DataBoundary loading={result.loading} error={result.error}>
        ...
        {runReceipt !== null && <Disclosure className="run-receipt" title="View Run Receipt" variant="code"><pre>{displayJson(runReceipt)}</pre></Disclosure>}
          ...
          >
            <EntityBrowser
              ...
              renderItem={(item, isSelected, select) => {
                ...
                return <SelectableRow type="button" role="option" selected={isSelected} aria-selected={isSelected} id={`monitor-index-${monitorId}`} className={`monitor-index-item ${isSelected ? "selected" : ""}`} onClick={() => select(monitorId)} key={monitorId}><span className="monitor-index-status"><Badge value={String(monitor.status ?? "UNKNOWN")} /></span><strong>{String(monitor.name ?? "Untitled Monitor")}</strong><small>{shortId(monitor.primary_instrument_id)} · {listOf<Dict>(monitor, "rules").length} rules</small><span className="monitor-index-health">{triggered > 0 ? `${triggered} triggered` : unavailable > 0 ? `${unavailable} unavailable` : "All quiet"}</span></SelectableRow>;
              }}
            />
          </Card>
                ...
                front={<div className="monitor-workspace-front">
                  <div className="monitor-title-row">
                    ...
                    <div className="monitor-title-actions">
                      {selectedDefinition.primary_instrument_id ? <LinkButton href={`/charts?instrument_id=${encodeURIComponent(String(selectedDefinition.primary_instrument_id))}${selectedDefinition.subject_id ? `&subject_id=${encodeURIComponent(String(selectedDefinition.subject_id))}` : ""}`}>Inspect Chart</LinkButton> : null}
                      ...
                      {String(selectedDefinition.status ?? "").toUpperCase() !== "ARCHIVED" && <Button variant="danger" className="monitor-delete-button" type="button" disabled={archivingId === String(selectedDefinition.monitor_id)} onClick={() => { void archiveMonitor(selectedDefinition); }}>{archivingId === String(selectedDefinition.monitor_id) ? "Archiving…" : "Archive"}</Button>}
                  ...
                  <section id="monitor-panel-overview" className="monitor-module-panel" role="tabpanel" aria-labelledby="monitor-tab-overview" hidden={activeModule !== "overview"}>
                    ...
                    <div className="monitor-overview-metrics"><div><span>Rules</span><strong>{selectedRules.length}</strong><small>Durable Definition</small></div><div><span>Triggered</span><strong>{selectedTriggeredCount}</strong><small>Active Alerts</small></div><div><span>Unavailable</span><strong>{selectedUnavailableCount}</strong><small>Needs Data Review</small></div><div><span>Events</span><strong>{visibleEvents.length}</strong><small>Transition History</small></div></div>
                    {selectedAttentionRules.length > 0 ? <div className="monitor-attention-list">{selectedAttentionRules.map(({ rule, state }) => <Button type="button" key={String(rule.rule_code)} onClick={() => setActiveModule("rules")}><Badge value={String(state.state ?? "NOT_EVALUATED")} /><div><strong>{String(rule.description ?? rule.rule_code)}</strong><span>{ruleCondition(rule)} · Current {String(state.observed_value ?? "N/A")}</span></div><span aria-hidden="true">→</span></Button>)}</div> : <p className="monitor-judgment-note">All deterministic rules are quiet in the latest known state.</p>}
                    {selectedJudgment.judgment_id ? <CompositeJudgmentCard judgment={selectedJudgment} /> : <p className="monitor-judgment-note">No composite judgment is available; deterministic rule states remain authoritative.</p>}
                  </section>
