...
test("research console is a responsive Research Subject/Thesis master-detail workspace", async () => {
  ...
  assert.match(source, /research-filters/);
  assert.match(source, /research-browser-next-rail/);
  assert.match(source, /research-filter-results/);
  assert.doesNotMatch(source, /research-browser-range|Showing Research Subjects/);
  ...
  assert.match(source, /research-subject-browser/);
  ...
  assert.match(styles, /\.research-filters \{ display:grid; grid-template-columns:/);
  assert.match(styles, /\.research-filters \{ display:grid; grid-template-columns:minmax\(240px,460px\) minmax\(160px,200px\) auto auto/);
  assert.match(styles, /\.page-action-list \{[^}]*position:absolute/);
  assert.match(source, /new ResizeObserver/);
  assert.match(source, /width >= 900\) setSubjectsPerPage\(6\)/);
  assert.match(source, /ref=\{researchIndexRef\}/);
  assert.match(styles, /\.research-subject-index-list \{ display:grid; grid-template-columns:repeat\(var\(--subjects-per-page,6\),minmax\(0,1fr\)\)/);
  assert.match(styles, /\.research-subject-browser \{ display:grid; grid-template-columns:28px minmax\(0,1fr\) 28px/);
  assert.match(styles, /@keyframes research-subjects-next/);
  assert.match(source, /slide-\$\{subjectPageDirection\}/);
...
test("overview Monitor titles deep-link to async-loaded definition cards", async () => {
  ...
  assert.match(monitorsSource, /visibleMonitorItems/);
