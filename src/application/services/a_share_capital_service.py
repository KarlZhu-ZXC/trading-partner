...
_CAPITAL_COMPONENT_ORDER = tuple(
    AShareComponentType(metric.value) for metric in CapitalMetricType
)
...
class AShareCapitalResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.ok:
            ...
            if any(
                item.component not in _CAPITAL_COMPONENT_ORDER
                for item in self.provenance
            ):
                raise DataContractError(
                    "capital failure provenance contains unrelated component"
                )
        ...
        async with asyncio.TaskGroup() as tg:
            for metric in ordered_metrics:
                tasks[metric] = tg.create_task(
                    settle_router_component(self._fetch_metric(
                        metric,
