"""Read-only Moomoo Notes cache synchronization CLI."""
...
async def _run(args: argparse.Namespace) -> int:
    async with application_container() as container:
        receipt = await container.services.external_notes.sync(
            analyze=False,
            source_code="MOOMOO_NOTE",
        )
        should_analyze = args.analyze or args.reanalyze_all
        analysis = (
            await container.services.external_notes.analyze_pending(
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="trading-partner-moomoo-notes-sync",
        description="Import private Moomoo Notes cache revisions without writing to Moomoo.",
    )
    parser.add_argument(
        "--analyze",
        action="store_true",
        help="After the fast source sync, analyze a bounded pending batch.",
    )
