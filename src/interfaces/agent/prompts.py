...
`view_get/inbox`，再对用户指定的 exact Note Revision 读取 `view_get/review`；用户询问某研究标的
当前已经确认的正式观点时，优先读取 `view_get/current`。这些操作只返回有界结构化内容，
不返回私人笔记全文，也不代表用户已经确认。除非用户明确要求核验某项当前市场事实，或
`view_get/review` 明确显示相关证据缺口，否则不要把行情查询作为观点复核的默认起点。
`research_workflow_run/evaluate_view` 是单独的 Provider evaluation，必须经过 capability-name
confirmation；它只追加非权威深度草稿，不能伪装成 `tp_read`，也不能作为任何正式判断的
用户确认。
