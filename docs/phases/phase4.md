...
Decision 才能形成 `ADOPTED` 或 `NO_ACTION`。`view_get/inbox|review|current` 是 bounded
durable reads；`research_workflow_run/evaluate_view` 是单独确认的 Provider evaluation，
只追加独立 review draft。Current View 从 exact Review + Decision 派生，
