...
3. composition root 第一轮拆分已完成：`bootstrap.py` 从 1049 行降至 961 行，新的回归上限
   ...
   1837 行的 `models.py` 分离，market structure、fundamentals、research/disclosure 模型也
   已独立，稳定 import façade 保留并收紧到 1050 行上限；snapshot 的
   Provider 结果校验也从 1480 行编排服务分离为 validation mixin，主服务收紧到 1100 行。
   下一步继续拆 capital / sentiment-options，并处理 1618 行的 DTO
   façade。
