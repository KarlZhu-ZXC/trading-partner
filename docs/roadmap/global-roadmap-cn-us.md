...
下列验收门槛是拟定目标，不是当前已测得的成绩。A/B 已交付；后续按 C → D 顺序推进；
每片先在现有契约上给出可检查的最小结果，再决定是否扩展。工作量为相对规模，不是工期承诺。

### 已交付基础：变化与原判断同屏

A 的实现与连续导航规则已移入 [当前产品说明](../product.md#changes-since-a-reviewed-decision)。
后续 B/C/D 复用该投影，不再创建第二份变化清单。生产读取的来源覆盖仍须逐项显示；
实施完成不表示所有历史资料都能补全。

### 已交付基础：有预算且可查证的研究过程

B 的运行模式、证据边界、预算与恢复规则已移入
[当前产品说明](../product.md#copilot-research-process)。后续改进应基于真实使用中暴露的
证据缺口，而不是增加角色数量。合成门禁通过不等于已完成真实模型质量或投资效果评估；
如需效果对比，须另行确定相同问题集、模型/预算配置及数据授权。
...
Research 的字段渲染、解释保留与无效修复问题已修复，并完成真实 Go 模型复测。
保留 [解释核验的边界](../operations/known-issues.md)，下一步交付 C 的图表与研究衔接。
